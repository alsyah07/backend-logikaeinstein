...
class subMapelService {
  ...
  async getSubMapelById(id) {
    try {
      const [rows] = await pool.query('SELECT * FROM sub_mapel LEFT JOIN mapel ON sub_mapel.id_mapel = mapel.id_mapel order by sub_mapel.orderby ASC WHERE sub_mapel.id_mapel = ?', [id]);
